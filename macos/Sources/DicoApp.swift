...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    /// ⌃1 gives odd characters on some layouts, so the digit has to come from the code.
    ...
    private func handlePanelKey(_ event: NSEvent) -> NSEvent? {
        ...
        // Tab / ⇧Tab walk the rail; ⌃1…⌃6 jump to a mode. One hand, and macOS
        // keeps its hands off both (⌘⇧3/4/5 are screenshots).
        if event.keyCode == 48, mods.isSubset(of: [.shift]) {
            model.cycleMode(mods.contains(.shift) ? -1 : 1)
            return nil
        }
        if mods == [.control], let n = AppDelegate.digitCodes[event.keyCode] {
...
func runSelfTest() -> Int32 {
    ...
    line("shortcuts catalogue + ⌘/ sheet") {
        ...
        let musts = ["⌘,", "⌘K", "Esc", "⌘1…⌘9", "Tab · ⇧Tab", "⌃1…⌃6", "⌘D", "⌘R", "⌘E",
